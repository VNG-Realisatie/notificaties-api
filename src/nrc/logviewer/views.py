...
class LogListView(ListView):
    ...
    queryset = Notificatie.objects.annotate(
        nr_of_subscribers=Count("notificatieresponse")
    ).order_by("-id")
...
class ResponseLogListView(ListView):
    ...
    def get_queryset(self):
        return NotificatieResponse.objects.filter(
            notificatie__id=self.kwargs["notificatie_id"]
        ).order_by("-id")
